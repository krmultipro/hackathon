...
export default class MenuScene extends Phaser.Scene {
    ...
    _createBackground(width, height) {
        ...
        this.heroPanel = this.add.rectangle(0, 0, 10, 10, MENU_THEME.panel, 0.76)
    ...
    _createHero() {
        ...
        this.heroHint = this.add.text(0, 0, 'Choisis ton defi du jour', {
    ...
    _createChip(label, color) {
        const bg = this.add.rectangle(0, 0, 10, 10, 0xffffff, 0.08)
    ...
    _createMenuCards() {
        this.courseCard = this._createActionCard({
            ...
            eyebrow: 'APPRENDRE',
            title: 'Cours',
            description: 'Des parcours clairs pour reviser vite et gagner en confiance.'
        });
        this.tournamentCard = this._createActionCard({
            accent: MENU_THEME.yellow,
            eyebrow: 'AFFRONTER',
            title: 'Tournoi',
            description: 'Des duels intenses pour tester tes reflexes et grimper au classement.'
        });
    ...
    _createActionCard({ accent, eyebrow, title, description }) {
        ...
        const panel = this.add.rectangle(0, 0, 10, 10, MENU_THEME.panelDeep, 0.92)
        ...
        container.add([shadow, panel, glow, accentBar, orb, eyebrowText, titleText, descriptionText, ctaText]);
        container.setSize(10, 10);
        container.setInteractive(new Phaser.Geom.Rectangle(-5, -5, 10, 10), Phaser.Geom.Rectangle.Contains);
    ...
    _createTopActions() {
        ...
        this.cornerLabel = this.add.text(0, 0, 'College Battle Mode', {
            font: 'bold 18px Arial',
            fill: '#c3d6ea'
        }).setOrigin(0, 0);
    }
    ...
    _createLeaderboardModal(width, height) {
        this.leaderboardOverlay = this.add.rectangle(0, 0, width, height, 0x03060f, 0.82)
            .setOrigin(0)
            .setDepth(40)
            .setVisible(false)
            .setInteractive();
        this.leaderboardPanel = this.add.rectangle(0, 0, 10, 10, MENU_THEME.panelDeep, 0.97)
            .setStrokeStyle(2, MENU_THEME.cyan, 0.9)
            .setDepth(41)
            .setVisible(false);
        this.leaderboardGlow = this.add.rectangle(0, 0, 10, 10, MENU_THEME.cyan, 0.08)
            .setDepth(41)
            .setVisible(false);
        this.leaderboardTitle = this.add.text(0, 0, 'Hall of Fame', {
            font: '900 38px Arial',
            fill: '#f7fbff'
        }).setOrigin(0.5).setDepth(42).setVisible(false);
        this.leaderboardSubtitle = this.add.text(0, 0, 'Top joueurs du college', {
            font: 'bold 18px Arial',
            fill: '#49d6ff'
        }).setOrigin(0.5).setDepth(42).setVisible(false);
        ...
        this.closeLeaderboardButton = this.add.text(0, 0, 'Fermer', {
            ...
            padding: { x: 16, y: 10 }
        }).setOrigin(0.5).setDepth(42).setVisible(false).setInteractive({ useHandCursor: true });
    }
    ...
    _attachEvents() {
        ...
        this.logoutButton.on('pointerout', () => this.logoutButton.setStyle({ fill: '#ffd7d1' }));
    ...
    _wireCard(card, onClick) {
        ...
        card.on('pointerdown', () => {
            const isMobile = this.scale.width < 960 || this.scale.height > this.scale.width;
            if (isMobile) {
                onClick();
                return;
            }
    ...
    _startAmbientMotion() {
        ...
        [this.courseCard, this.tournamentCard, this.leaderboardCard].forEach((card, index) => {
            this.tweens.add({
                targets: card,
                y: `-=${2 + index}`,
    ...
    _layout(width, height) {
        ...
        const heroHeight = Math.min(height * (isMobile ? 0.18 : 0.3), isMobile ? 132 : 270);
        const cardWidth = Math.min(isMobile ? width * 0.9 : width * 0.25, 340);
        const cardHeight = isMobile ? 118 : 220;
        const titleSize = Phaser.Math.Clamp(Math.round(base * (isMobile ? 0.13 : 0.095)), 34, 84);
        const subtitleSize = Phaser.Math.Clamp(Math.round(base * (isMobile ? 0.034 : 0.03)), 13, 26);
        const cardTitleSize = Phaser.Math.Clamp(Math.round(base * (isMobile ? 0.06 : 0.04)), 22, 36);
        const cardTextSize = Phaser.Math.Clamp(Math.round(base * (isMobile ? 0.024 : 0.024)), 11, 20);
        const chipTextSize = Phaser.Math.Clamp(Math.round(base * (isMobile ? 0.027 : 0.022)), 13, 18);
        const modalWidth = Math.min(width * 0.84, 620);
        const modalHeight = Math.min(height * 0.74, 560);

        this._drawBackground(width, height);

        this.heroPanelShadow
            .setPosition(this._snap(width / 2 + 8), this._snap(height * (isMobile ? 0.19 : 0.26) + 10))
            .setSize(heroWidth, heroHeight);
        this.heroPanel
            .setPosition(this._snap(width / 2), this._snap(height * (isMobile ? 0.19 : 0.26)))
            .setSize(heroWidth, heroHeight);

        this.titleText
            .setPosition(this._snap(width / 2), this._snap(height * (isMobile ? 0.16 : 0.22)))
            .setStyle({ font: `900 ${titleSize}px Arial` });
        this.subtitleText
            .setPosition(this._snap(width / 2), this._snap(height * (isMobile ? 0.215 : 0.285)))
            .setStyle({
                font: `${subtitleSize}px Arial`,
                wordWrap: { width: Math.min(heroWidth - (isMobile ? 36 : 80), 760) }
            });
        this.heroHint
            .setPosition(this._snap(width / 2), this._snap(height * (isMobile ? 0.285 : 0.35)))
            .setStyle({ font: `bold ${Math.max(isMobile ? 12 : 16, Math.round(base * 0.026))}px Arial` });

        this._layoutChips(width, height, isMobile, chipTextSize);
        this._layoutCards(width, height, isMobile, cardWidth, cardHeight, cardTitleSize, cardTextSize);

        this.logoutButton
            .setPosition(this._snap(width - 18), this._snap(isMobile ? 16 : 20))
            .setStyle({ font: `bold ${Math.max(isMobile ? 12 : 14, Math.round(base * 0.023))}px Arial` });
        this.cornerLabel
            .setPosition(this._snap(18), this._snap(isMobile ? 18 : 24))
            .setStyle({ font: `bold ${Math.max(isMobile ? 12 : 14, Math.round(base * 0.022))}px Arial` });
    ...
    _drawBackground(width, height) {
        ...
        this.bgGlow.fillCircle(width * 0.18, height * 0.2, Math.min(width, height) * 0.2);
        this.bgGlow.fillStyle(MENU_THEME.coral, 0.07);
        this.bgGlow.fillCircle(width * 0.84, height * 0.18, Math.min(width, height) * 0.16);
        this.bgGlow.fillStyle(MENU_THEME.yellow, 0.04);
        this.bgGlow.fillCircle(width * 0.5, height * 0.82, Math.min(width, height) * 0.2);
    ...
    _layoutChips(width, height, isMobile, chipTextSize) {
        ...
        this.statChips.forEach((chip, index) => {
            ...
            chip.bg.setPosition(this._snap(x), this._snap(y)).setSize(chipWidth, chipHeight);
            chip.dot.setPosition(this._snap(x - (chipWidth / 2) + (isMobile ? 12 : 22)), this._snap(y));
            chip.text
                .setPosition(this._snap(x + (isMobile ? 4 : 8)), this._snap(y))
                .setStyle({ font: `bold ${isMobile ? Math.max(10, chipTextSize - 3) : chipTextSize}px Arial` });
        });
    }

    _layoutCards(width, height, isMobile, cardWidth, cardHeight, cardTitleSize, cardTextSize) {
        ...
        [this.courseCard, this.tournamentCard, this.leaderboardCard].forEach((card, index) => {
            const { shadow, panel, glow, accentBar, orb, eyebrowText, titleText, descriptionText, ctaText, title } = card.cardParts;
            ...
            orb.setPosition(this._snap((cardWidth / 2) - (isMobile ? 32 : 52)), this._snap(-(cardHeight / 2) + (isMobile ? 34 : 52)));
            eyebrowText
                .setPosition(this._snap(-(cardWidth / 2) + (isMobile ? 28 : 34)), this._snap(-(cardHeight / 2) + (isMobile ? 16 : 28)))
                .setStyle({ font: `900 ${Math.max(isMobile ? 10 : 12, Math.round(cardTextSize * 0.75))}px Arial` });
            titleText
                .setPosition(this._snap(-(cardWidth / 2) + (isMobile ? 28 : 34)), this._snap(-(cardHeight / 2) + (isMobile ? 36 : 58)))
                .setStyle({
            ...
            descriptionText
                .setPosition(
                    ...
                    this._snap(-(cardHeight / 2) + (isMobile ? (title === 'Classement' ? 54 : 58) : 108))
                )
                .setStyle({
                    font: `${Math.max(isMobile && title === 'Classement' ? cardTextSize - 1 : cardTextSize, 10)}px Arial`,
                    wordWrap: { width: cardWidth - (isMobile ? (title === 'Classement' ? 118 : 96) : 86) }
                });
            ctaText
                .setPosition(this._snap(-(cardWidth / 2) + (isMobile ? 28 : 34)), this._snap((cardHeight / 2) - (isMobile ? 24 : 46)))
                .setStyle({ font: `bold ${Math.max(isMobile ? 13 : 16, Math.round(cardTextSize))}px Arial` });
        });
    }
    ...
    _layoutLeaderboardModal(width, height, modalWidth, modalHeight, base) {
        ...
        this.leaderboardTitle
            .setPosition(this._snap(width / 2), this._snap(height / 2 - modalHeight * 0.36))
            .setStyle({ font: `900 ${Math.max(28, Math.round(base * 0.05))}px Arial` });
        this.leaderboardSubtitle
            .setPosition(this._snap(width / 2), this._snap(height / 2 - modalHeight * 0.27))
            .setStyle({ font: `bold ${Math.max(15, Math.round(base * 0.022))}px Arial` });
        this.leaderboardContent
            .setPosition(this._snap(width / 2), this._snap(height / 2 - modalHeight * 0.18))
            .setStyle({
                font: `${Math.max(16, Math.round(base * 0.03))}px Arial`,
                wordWrap: { width: modalWidth - 90 }
            });
        this.closeLeaderboardButton
            .setPosition(this._snap(width / 2), this._snap(height / 2 + modalHeight * 0.39))
            .setStyle({ font: `bold ${Math.max(16, Math.round(base * 0.024))}px Arial` });
    }
    ...
    async openLeaderboard() {
        ...
        try {
            ...
            const lines = players.map((player, index) => {
                const prefix = index === 0 ? '1. Champion' : `${player.rank || index + 1}.`;
