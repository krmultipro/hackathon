...
@RestController
@RequestMapping("/api/match-answers")
...
public class MatchAnswerController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Réponse trouvée", content = @Content(schema = @Schema(implementation = MatchAnswer.class))),
                        @ApiResponse(responseCode = "404", description = "Réponse non trouvée")
        })
        @GetMapping("/{id}")
        public ResponseEntity<MatchAnswer> getById(
                        @Parameter(description = "Identifiant de la réponse", example = "1") @PathVariable Long id) {
        ...
        @PostMapping
        public ResponseEntity<MatchAnswer> create(@Valid @RequestBody MatchAnswer matchAnswer) {
        ...
        public ResponseEntity<MatchAnswer> update(
                        @Parameter(description = "Identifiant de la réponse", example = "1") @PathVariable Long id,
                        @Valid @RequestBody MatchAnswer details) {
